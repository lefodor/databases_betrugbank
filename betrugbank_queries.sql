...
select
	dc.country_iso,
	count( distinct 
		case when dat.name in ('bankaccount', 'savingaccount') then dc.id 
			else null end ) as cnt_deposit_customers,
	sum( case when dat.name in ('bankaccount', 'savingaccount') then fa.balance 
			else 0 end ) as sum_deposit
from
	dm.f_account fa
inner join dm.d_account da on
	da.account_number = fa.account_number
...
from
	dm.f_account fa
inner join dm.f_arrears far on
	far.date_id = fa.date_id
	and far.account_number = fa.account_number
inner join dm.d_date dd on
	dd.id = fa.date_id
inner join dm.b_cust_acct bca on
	bca.account_number = fa.account_number
...
select
	date,
	avg(exp(intercept + var1 + var2 + var3 + var4) / 
...
from
	...
	inner join dm.d_account da on
		da.account_number = bca.account_number
	inner join dm.d_account_type dat on
		dat.id = da.account_type_id
	where
		dd.date >= '2021-03-31'
...
select
	date,
	sum( balance * ( exp(intercept + var1 + var2 + var3 + var4) / 
		(1 + exp(intercept + var1 + var2 + var3 + var4))) ) / sum(balance) as pd_wgt
from
	(
	select
		dd.date,
...
select top 1
	dc.id as customer_id
from
	dm.f_transactions ft
inner join dm.d_date dd on
	dd.id = ft.date_id
inner join dm.b_cust_acct bca on
	bca.account_number = ft.account_number
inner join dm.d_customer dc on
	dc.id = bca.customer_id
where
	dd.date >= '2021-04-01'
...
inner join dm.d_date dd on
	dd.id = fa.date_id
inner join dm.d_interest_rate dir on
	dir.id = fa.interest_rate_id
inner join dm.b_cust_acct bca on
	bca.account_number = fa.account_number
where
	dd.date = '2021-04-30'
group by
	bca.customer_id
order by
	max(dir.interest_rate) desc;

-- ----------------------------------------- q#16 ---------------------------------------
select
	avg(dir.interest_rate) as avg_int_rate
from
	dm.f_account fa
inner join dm.d_date dd on
	dd.id = fa.date_id
