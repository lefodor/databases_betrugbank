...
from
	...
	from
		...
		date::date
	from
		generate_series('1900-01-01'::date, '2050-12-31'::date, '1 day'::interval) as t(date) ) t
...
select
	...
	case
		when ( random() * (1-0) + 1) :: int = 1 then -1
		else ( random() * (55100-44800) + 44800) :: int
	end as closing_date_id
from
	dm.b_cust_acct;
...
create table dm.b_cust_acct (
	...
	account_number int primary key,
	customer_role char(10) not null
);

insert
	into
	dm.b_cust_acct (customer_id,
	account_number,
	customer_role)
values 
(10001,
...
alter table dm.f_account add primary key ( date_id,
account_number );
...
from
	...
	select
		...
		case
			...
			when snap = 2 then 44346
		end as date_id,
		(
		case
			when snap = 0 then '2021-03-31'
			when snap = 1 then '2021-04-30'
			when snap = 2 then '2021-05-31'
		end ) :: date as date,
		case
			when da.account_type_id in (1, 2) then tmp_lt0.ob
			else tmp_gt0.ob
		end as ob
	from
		(
		select
			*,
			( random() * (2e7-1e5) - 3e7 ) :: int as ob
		from
			dm.b_cust_acct ) tmp_lt0
	inner join (
		select
			*,
			( random() * (2e7-1e5) + 1e6 ) :: int as ob
		from
			dm.b_cust_acct ) tmp_gt0 on
		tmp_gt0.customer_id = tmp_lt0.customer_id
		and tmp_gt0.account_number = tmp_lt0.account_number
	inner join dm.d_account da on
		da.account_number = tmp_lt0.account_number
	inner join dm.d_account_type dat on
		da.account_type_id = dat.id
	cross join (
		select
			snap
		from
			generate_series(0, 2) as snap) s
) acct
left join 
(
	select
		distinct (date_trunc('month', dd.date) + interval '1 month - 1 day')::date as end_of_month,
		ftr.account_number,
		sum(case when (date_trunc('month', dd.date) + interval '1 month - 1 day')::date = '2021-03-31' 
				then 0 
				else ftr.amount end) 
				over (partition by ftr.account_number
	order by
		ftr.account_number,
		(date_trunc('month', dd.date) + interval '1 month - 1 day')::date) as cum_balance_delta
	from
		dm.f_transactions ftr
	inner join dm.d_date dd 
on
		ftr.date_id = dd.id ) as cb on
	cb.account_number = acct.account_number
	and cb.end_of_month = acct.date
order by
	2,
	1;
...
alter table dm.f_expected_payment add primary key ( date_id,
account_number );
...
from
	(
	select
		(date_trunc('month', dd.date) + interval '1 month - 1 day')::date as end_of_month,
		ftr.account_number,
...
alter table dm.f_arrears add primary key ( date_id,
account_number );
...
from
	dm.f_expected_payment
where
	expected_payment - effective_payment != 0;
...
alter table dm.f_scoring add primary key ( date_id,
customer_id );
...
inner join dm.d_scorecard ds on
	ds.id = dc.scorecard_id
where
	dd.id in (44285, 44315, 44346) ;

/*
select
	fs.*,
	exp(ds.intercept + fs.var1 + fs.var2 + fs.var3 + fs.var4) / 
		(1 + exp(ds.intercept + fs.var1 + fs.var2 + fs.var3 + fs.var4)) as prob
from
	dm.f_scoring fs
inner join dm.d_scorecard ds on
...
order by
	exp(ds.intercept + fs.var1 + fs.var2 + fs.var3 + fs.var4) / 
		(1 + exp(ds.intercept + fs.var1 + fs.var2 + fs.var3 + fs.var4)) desc;
*/
